...
export const itemResize_mousemove: Function = throttle((ev: MouseEvent) => {
  ...
  const {w: beforeW, h: beforeH} = fromItem.pos
  ...
  const limitGridContainer = ({w, h}) => {
    ...
    if (fromItem.resizeOut) {
      if ((w + pos.x) > col) w = col - pos.x + 1     //item调整大小时在容器右边边界超出时进行限制
    }
    if (w < pos.minW) w = pos.minW
    if (w >= pos.maxW && pos.maxW !== Infinity) w = pos.maxW
    if (fromItem.resizeOut) {
      if ((h + pos.y) > row) h = row - pos.y + 1
    }
  ...
  let newResize = limitGridContainer(resized)    //当前鼠标距离x,y的距离构成的矩形
  ...
  const resizeSpaceLimit = ({w, h}): { w: number, h: number } => {
    ...
    const maxBlankMatrixLimit = container.layoutManager.findStaticBlankMaxMatrixFromItem(container.engine.items, fromItem)
  ...
  if (!tempStore.isBlockResize) newResize = resizeSpaceLimit(newResize)
  if (!newResize) return
  tempStore.newResizeW = newResize.w
  tempStore.newResizeH = newResize.h
  //----------------------------------------------------------------------------------------//
  if (beforeW !== newResize.w || beforeH !== newResize.h) { // 只有改变Item的大小才进行style重绘
