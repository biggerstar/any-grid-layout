 ...
 *  itemResizing(w,h,item)                                 item每次大小被改变时
 *  itemResized(w,h,item)                                  item在鼠标抬起后在容器中的最终大小
 *  itemMoving(nowX,nowY,item)                             item拖动时在容器内所属位置的nowX和nowY，如果鼠标在容器外,则nowX和nowY是容器边缘最大最小值,不会是超过或者是负数
 *  itemMoved(nowX,nowY,item)                              item拖动结束时在容器内最终位置的nowX和nowY，如果鼠标在容器外,则nowX和nowY是容器边缘最大最小值,不会是超过或者是负数
 ...
 *  containerSizeChange(oldSize,newSize,container)         内层容器(grid-container)大小改变触发的事件,oldSize和newSize包含以下信息{ containerW,containerH,row,col,width,height }
