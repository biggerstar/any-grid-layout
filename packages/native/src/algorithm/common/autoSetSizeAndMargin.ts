...
export function autoSetSizeAndMargin(container, isSetConfig: boolean = false): {
  ...
  margin[0] = getFirstNumber(marginX, margin[0], marginY)   // 优先获取 marginX，没定义的话再获取margin[0]，如果还没定义的话直接看marginY是否定义并获取
  margin[1] = getFirstNumber(marginY, margin[1], marginX)
  size[0] = getFirstNumber(sizeWidth, size[0], sizeHeight)
  size[1] = getFirstNumber(sizeHeight, size[1], sizeWidth)
