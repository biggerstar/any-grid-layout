...
export class LayoutManager {
  ...
  public each: (next: (curRow: number, curCol: number) => any, layoutManager: LayoutManager) => void
   ...
   * ```
   * */
  public validate(col: number, customLayout: Partial<LayoutManagerSetStateOptions>): Partial<ThrowMessageEvent> | null {
  ...
  private reset() {
    const col = this.col
    const row = Math.max(this.minimumRows, this.row)
    this.layoutMatrix = []
    for (let i = 0; i < row; i++) {
      this.layoutMatrix[i] = []
      for (let j = 0; j < col; j++) {
  ...
  public autoAnalysis(): AnalysisResult {
    ...
    let cont: number = this.maximumRows  // 最大拓展行数的边界
    do {
      if (!cont--) {
        break
      }
  ...
  /**
   * 遍历任意方向的矩阵，point1和point2必须是左上和右下对角点
   * */
  public eachMatrix(fn: (x: number, y: number) => any): void {
    if (!isFunction(this.each)) {
      throw new Error('请先实现矩阵遍历算法 each 函数， 并通过 setState 进行设置')
    }
    this.each((x: number, y: number) => <any>fn && fn.call(null, x, y), this)
  }

  public static validate(col: number, customLayout: Partial<LayoutManagerSetStateOptions>): Partial<ThrowMessageEvent> | null {
    if (!isNumber(col)) {
