...
export class Item extends ItemGeneralImpl {
  ...
  private readonly __default: CustomItem
  ...
  constructor(itemOption: CustomItem) {
    ...
    this.__default = Object.freeze(new ItemGeneralImpl())
  }
  ...
  public mount() {
    ...
    if (isString(this.el)) {
      this.itemElement = <HTMLElement>document.querySelector(this.el)
    } else if (this.el) {
      this.itemElement = this.el.isConnected ? <HTMLElement>document.adoptNode(this.el) : <HTMLElement>this.el
    }
    if (!this.itemElement) {
      this.itemElement = document.createElement("div")
    }
    this.itemElement.classList.add(grid_item_content)
    this.element.appendChild(this.itemElement)
    this.container.containerElement.appendChild(this.element)
  ...
  public offsetLeft(): number {
    const {gapX, itemWidth} = this.container.useLayout
  ...
  public offsetTop(): number {
    const {gapY, itemHeight} = this.container.useLayout
  ...
  public offsetRight(): number {
    const {gapX, itemWidth} =this.container.useLayout
    const col = this.container.useLayout.col
  ...
  public offsetBottom(): number {
    const {gapY, itemHeight} = this.container.useLayout
    const row = this.container.useLayout.row
  ...
  //   const coverRightItems = manager.findCoverItemsFromPosition(this.container.useLayout.items, {
  //     ...this.pos,
  //     w: this.container.useLayout.col - this.pos.x + 1
  //   }, [this])
  ...
  //   const {gapY, itemHeight} = this.container.useLayout
  //   const manager = this.container.layoutManager
  //   const coverBottomItems = manager.findCoverItemsFromPosition(this.container.useLayout.items, {
  //     ...this.pos,
  //     h: this.container.useLayout.row - this.pos.y + 1
  //   }, [this])
