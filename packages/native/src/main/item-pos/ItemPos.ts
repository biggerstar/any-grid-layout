...
export class ItemPos extends ItemPosGeneralImpl {
  // public i?: number
  public __default?: ItemPosGeneralImpl  // 框架默认配置
  public customOptions?: CustomItemPos

  constructor(pos: CustomItemPos) {
    super()
    this.__default = Object.freeze(new ItemPosGeneralImpl()) // 必须在_defineXXX 之前
    this.customOptions = pos
    this.defineSyncCustomOptions(pos)
    merge(this, pos, false, ['x', 'y', 'w', 'h'])   // 1.先加载minX,maxX...等限制后
    merge(this, pos)  //  2. 再合并所有且受上面 minX,maxX 限制
  }
  ...
  public defineSyncCustomOptions(_customOptions: CustomItemPos) {
    const self = this
    const __default = this.__default
    ...
    const getCurPosValue = (k) => {
      ...
      return curCustomOptions.hasOwnProperty(k) ? curCustomOptions[k] : (_tempPos[k] || __default?.[k])
    }
    ...
    const set = (k: keyof ItemPosGeneralImpl, v: any) => {
      ...
      if (['w', 'h', 'x', 'y'].includes(<string>k)) {  // 如果是x,y,w,h外面没有指定则不会修改用户传入配置
        ...
        if (!isNumber(v) || isNaN(v) || !isFinite(v)) {
          delete curCustomOptions[k]
        }
   ...
   * */
  public getComputedCustomPos(): CustomItemPos {
    const result = {}
    for (const name in this.customOptions) {
      result[name] = this[name]
    }
    return result as CustomItemPos
  }
}
